...
// TODO: Is this ok?
    ...
    // Parsing of the SVG file to generate a path
    // ==========================================
    String xmlString = String::fromUTF8 (BinaryData::_571_svg, BinaryData::_571_svgSize);
    ScopedPointer<XmlElement> xml571 = XmlDocument::parse (xmlString);
    
    // This is ugly and specific to this SVG
    if (xml571 != nullptr && xml571->hasTagName("svg"))
    {
        XmlElement* svgGroup = xml571->getFirstChildElement();
        
        while (svgGroup != nullptr)
        {
            if (svgGroup->hasTagName ("g"))
            {
                XmlElement* pathOrGroup = svgGroup->getFirstChildElement();
                
                while (pathOrGroup != nullptr)
                {
                    if (pathOrGroup->hasTagName ("path"))
                    {
                        path571.addPath (Drawable::parseSVGPath (pathOrGroup->getStringAttribute ("d")));
                    }
                    else if (pathOrGroup->hasTagName ("g"))
                    {
                        XmlElement* pathXml = pathOrGroup->getFirstChildElement();
                        
                        while (pathXml != nullptr)
                        {
                            if (pathXml->hasTagName ("path"))
                            {
                                path571.addPath (Drawable::parseSVGPath (pathXml->getStringAttribute ("d")));
                            }
                            
                            pathXml = pathXml->getNextElement();
                        }
                    }
                    
                    pathOrGroup = pathOrGroup->getNextElement();
                }
            }
